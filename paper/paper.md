---
title: 'kmhelpers: A Python toolkit for automated management of $k$-mer indices'
tags:
  - Python
  - bioinformatics
  - k-mer indexing
...
bioinformatics. $k$-mer indices built on Bloom filters, such as those
produced by `kmindex` [@lemane2024], allow efficient querying of large sequence collections. However, building and managing
such indices in practice involves a complex chain of steps: enumerating sample
files, counting $k$-mers, selecting appropriate Bloom-filter parameters,
generating index definitions, orchestrating build jobs, querying indices, and
compressing outputs for long-term storage. Each step requires expertise, and
errors at any stage can invalidate downstream results or waste significant
compute time.

`kmhelpers` is an open-source Python toolkit that automates this entire
workflow. It provides both a command-line interface (CLI) and a Python API,
covering every stage of the $k$-mer index lifecycle:
...
**Parameter selection.** Bloom filter-based indices require choosing a filter
...
**Workflow orchestration.** Building a large index may involve hundreds to millions of samples spread across multiple sub-indices. Managing
